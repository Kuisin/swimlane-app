...
| `section (名前) #色` … `end-section` | 詳細ブロック（本流は内部をスキップ） | 色付き点線枠のグループ |
...
## 詳細ブロック（section / end-section）

本流の矢印は `section` と `end-section` の**間をスキップ**し、次の手順へそのまま進みます。内部のステップは詳細用として表示され、グループ内だけで順に接続されます（監査ログ・補足処理など、省略可能な詳細向け）。
...
- `#色名`（任意）で枠色を指定できます。色名は `if ... than #色名` と同じです。
- 内部の行は半角スペース2つでインデントするのが推奨です。
- `if` ケース内や `fork` パス内にも書けます。ケースの本流も内部をスキップします。
- `end-section` の直後は、通常の手順と同様に次のブロックへ合流します。
