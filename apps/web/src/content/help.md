...
- `id: 名前;` — ステップの一意 ID（`merge` の合流先。ファイル全体で重複不可）
- `label: 名前;` — 左カラム用の表示名
...
分岐ケースの末尾に `merge <id>;` を置くと、そのケースは `endif` のマージダイヤモンドへ進まず、`id:` が一致する**下流のステップへ直接合流**する矢印を描きます。ケースごとに合流先が異なる場合（例: キャンセル時だけ通常処理を飛ばして終端へ合流）に使います。`if` の外では使えません。
