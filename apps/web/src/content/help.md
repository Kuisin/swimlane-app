...
| `start-point` … `end-point` | 詳細ブロック（本流は内部をスキップ） | 点線枠のグループ |
...
## 詳細ブロック（start-point / end-point）

本流の矢印は `start-point` と `end-point` の**間をスキップ**し、次の手順へそのまま進みます。内部のステップは詳細用として表示され、グループ内だけで順に接続されます（監査ログ・補足処理など、省略可能な詳細向け）。
...
- `start-point` で開始、`end-point` で終了（ペア必須。未閉じはエラー）。
- 内部の行は半角スペース2つでインデントするのが推奨です。
- `if` ケース内や `fork` パス内にも書けます。ケースの本流も内部をスキップします。
- `end-point` の直後は、通常の手順と同様に次のブロックへ合流します。
