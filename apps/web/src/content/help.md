...
| `merge: <id>;` | ケース末尾。`endif` を使わず `id:` の下流へ合流 | 破線の前方矢印（`if` 内のみ） |
...
`if` ケースの末尾に `merge: <id>;` を置くと、そのケースは `endif` の合流ダイヤモンドへ進まず、**下流**のステップで `id: <id>;` が一致するブロックへ破線矢印で直接合流します。キャンセル時だけ終端へ飛ばす、といったケース向けです。`if` の外では使えません。
