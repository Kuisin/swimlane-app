...
export function Diagram({
  ...
  // stay inside it instead of spilling past the diagram's right edge.
  ...
  function computeFrameLayout(frame) {
    ...
    casesByLane.forEach((indices, laneId) => {
      const caseExtents = indices.map((ci) => {
        ...
        let mn = 0;
        let mx = 0;
        // The case's own steps in this lane (block + side doc chips) must be
        // cleared by adjacent sibling cases, so a sibling's arrow never crosses
        // a doc chip.
  ...
  /**
   * Actual visual extents past the step center, matching how the block and its
   * doc chips are drawn. Used for branch spacing so a sibling case (and its
   * fan-out/merge arrow) clears a neighbor's doc chips.
   */
  ...
  function buildLoopBackPath({
    ...
    /**
     * Keep the loop rail inside the role lane grid — never in the left gutter
     * or past the rightmost lane. (The diagram-wide xPad bounds let it drift
     * into the gutter, outside any role.)
     */
    const lastLaneIdx = lanes.length - 1;
    const laneGridLeft = laneX(0);
    const laneGridRight =
  ...
  if (lanes.length > 0 && lastStepRowIndex >= 0) {
    rows.forEach((row, i) => {
      ...
      // The merge diamond closing an if/fork.
      if (row.kind === "branchEnd") {
        if (meta == null) return;
      ...
      // Empty placeholder step ([role:] with no block).
      if (row.kind === "step" && row.empty) {
        if (meta == null) return;
        if (next?.kind === "step" && next.skipIndex) return;
        if (dividerLandsOnGroupEnd(i)) return;
        const base = meta.y + (stepRowHeightByIndex.get(i) ?? rowH);
